...
as well as continuous wave signals, each accompanied by comprehensive discussion. 
\cite{Richards:2005} also hits on general waveform techniques for radar with discussions on frequency modulation 
and pulse trains, but more of a focus on radar signal processing.
...
velocity of various targets.

Some wave lengths perform better for observing weather than others, especially in respect to attenuation 
...
somewhat mitigated by, e.g. using polarimetric radar measurements \citep{Ryzhkov:2008}. One advantage of higher frequency
radars is the need for a smaller antenna size for the same angular resolution. Other components such as waveguides,
amplifiers and support structure are also smaller, so the whole system requires less space.
...
Phased array radars move away from standard dish antennas to so-called array antennas.
An array antenna is made up of discrete units of transceivers distributed in one or more dimensions. The NWRT Phased Array 
Radar, for example, has over 4352 elements distributed across a plane \citep{Cheong:2008}. Each of the elements is able to transmit a
...
Klystron. One way to regain the sensitivity lost due to lower transmit power is to transmit longer pulses. Longer pulses,
in turn, decrease range resolution,
...
\Delta r = \frac{c \tau}{2},
\end{equation}
where $c= \SI{3e8}{\metre\per\second}$ represents the speed of light and $\tau$ is the pulse length. Looking at Equation
\ref{eq:rangeres} it can be seen that the longer the pulse, the larger the resolution volume will be. 
Pulse compression is a technique to increase the SNR and resolution by using waveforms that allow controlling the duration
and bandwidth separately.

Compression is achieved by using one or more matched filters since their output is the autocorrelation function 
of the transmitted signal. This allows the design of waveforms such that they are narrowly focused in a 
