...
amplitude tapering is applied. For a phase code, the amplitude window is not applied to the whole waveform, but each
subpulse is tapered separately. The waveform will look like the one depicted in Figure \ref{fig:barker13-15us-realsignal}. 

\begin{figure}[H]
\includegraphics[width=\imgsize]{figures/barker-15us-realsignal.png}
\caption{Figure of a tapered Barker 13 waveform. Each subpulse is tapered separately, unlike non-phase coded
waveforms that are tapered across the whole pulse length.}
\label{fig:barker13-15us-realsignal}
\end{figure}

However, performance calculations show that tapering does not improve the sidelobe levels.  Due to the Barker codes
...
stay the same, while range resolution improves moderately with increasing $\beta$.  As expected, the PPR shape is
consistent with other results since it only depends on the Kaiser window, not the waveform.

The improvement in range resolution is shown in Figure \ref{fig:barker13-15us-opti-slice3}. Unfortunately,
sidelobes still occur at around \SI{-22}{\decibel}, so there is no improvement and similar to the untapered Barker code,
its usefulness for weather radar applications is limited.
...
\includegraphics[width=\imgsizes]{figures/barker-15us-opti-slices.png}
\caption{Zero-Doppler slices of Barker 13 AFs showing the improvement in range resolution with tapering. 
ISL and PSL do not change significantly.}
...
by the system impulse response. For simplification, the components in the receive chain up to and excluding the matched
filter are represented by some equivalent system with impulse response $g(t)$, shown in Figure
\ref{fig:distortblock}. The effects of that on the waveform are presented here. A couple of
...
which is very likely not accurate. However, doing this will exaggerate the effect and make it more easily visible, which is
the goal of this section. 

\begin{figure}[H]
\includegraphics[width=\imgsize]{figures/distortblock.png}
\caption{A very simplified block diagram of the receive chain. All components except the matched filter are represented
by an equivalent system with impulse response $g(t)$, with the matched filter impulse response denoted as $h(t)$.}
...
transmitter chain, so that, on receive, the effect of $g(t)$ is cancelled out, because $g(t) \convolution g^{-1}(t) =
