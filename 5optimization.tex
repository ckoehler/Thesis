...
For comparison, Figure \ref{fig:lfm-isl-0d-15us-opti} shows the same optimizations done and presented in Figure
\ref{fig:lfm-isl-15us-opti}, but only across the zero-Doppler slice of the AF, not the whole AF. There is no significant
difference, so using the whole AF for calculations does not adversely affect the calculations. Thus, to be
comprehensive, the rest of the optimizations in this work are made across the whole AF.

\begin{figure}[H]
\includegraphics[width=\imgsize]{figures/lfm-isl-0d-15us-opti.png}
\caption{Optimization results for LFM with Kaiser amplitude tapering and $\tau=\SI{15}{\micro\second}$. Here, only the
zero Doppler cut is examined, not the whole AF.  }
\label{fig:lfm-isl-0d-15us-opti}
\end{figure}
