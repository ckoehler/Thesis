...
TODO: show 0 doppler and 50 doppler in one plot.
...
\section{Barker Code}
In this section, a Barker 13 code of different pulse lengths will be examined. Figures \ref{fig:barker13-15us}
...
\caption{Amplitude tapering optimization results for a Barker 13 code. There is not much improvement compared to the
standard Barker phase code.  }
...
\end{figure}
TODO: show comparison to normal plot.


TODO: taper each subpulse.
...
\ref{fig:lfm-200us-0d} show the zero-Doppler cut of both signals.  As expected, the sidelobes are approximately
...
PSL, because the AF is essentially one large mainlobe with only minimal sidelobes. TODO: show example.
...
Generally, a system impulse response has no significant effect on the Doppler distortion for weather radar
applications. However, it does affect the range delay by smoothing out the waveform similar to amplitude tapering,
lowering range resolution. The distortion is not symmetric because the system impulse response is not, which is
...
TODO: discuss how and add block diagram.
