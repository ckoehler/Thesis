...
Applying amplitude tapering to the Barker 13 phase coded signal does not improve the signal. Due to the Barker codes being
perfect this isn't surprising. Specifically, Figure \ref{fig:barker13-15us-opti} shows the optimization results of applying a 
...
ISL does become slightly better with very light tapering, with the minimum ISL occuring with $\beta = 1.026$. The zero-Doppler cut of
...
so there is not much improvement. These results show that a Barker coded signal is not very useful for weather radar applications.
