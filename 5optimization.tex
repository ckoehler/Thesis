...
that the effect of pulse length on Doppler resistance can be examined. As predicted in Section \ref{s:dopplereffect}, if the 
...
\section{Barker code}
Next, a length 13 Barker code of different pulse lengths will be examined. As with the pulse, the Doppler tolerance
decreases with a longer pulse.
...
Comparing the two AFs to determine the difference that the pulse length makes, it can be seen that it has
little effect for weather applications. To get a better understanding of the difference 
in range resolution, the zero Doppler cut of both AFs is shown in figures \ref{fig:lfm-15us-0d} and \ref{fig:lfm-200us-0d}.

As expected, the sidelobes are around -13 dB in both AFs, and the shapes are approximately the same
