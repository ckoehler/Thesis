...
The simulations are done in multiple parts. First, a waveform is generated based on a given
...
the new pulse length $T$ which will be approximately twice the original $T$. That lengthening
...
In pseudo-code, given the complex waveform \emph{signal} and an array of Doppler frequencies \emph{doppler\_frequencies},
...
A rectangular pulse is the simplest waveform for a radar. The following figures show the AF for pulses of varying lengths so 
that the effect of pulse length on Doppler resistance can be examined. Figure \ref{fig:simplepulse-1us} shows the AF
of a \SI{1}{\micro\second} pulse. 
There is no visible change in the AF across the Doppler axis, so the different Doppler cuts are identical, which means that for weather radar 
...
it to the shorter pulse, it looks identical. There is some distortion at higher
Doppler velocities for the longer pulse, but it is so minimal that it is not visible, making the impact on weather radar application negligible.
Additionally, pulse lengths of \SI{200}{\micro\second} are not common. For comparison, the two pulse lengths used in the WSR-88D radar network are
\SI{1.57}{\micro\second} and \SI{4.57}{\micro\second} \citep[p. 47]{Doviak:2006}.
...
confirming again that for weather radar applications, the Doppler distortions can be neglected.
...
Comparing these images to Figures \ref{fig:barker13-200us} and \ref{fig:barker13-200us-0D} again shows no visible difference
in the Doppler dimension, so the pulse length has no effect on the shape of the AF for a Barker 13 code, besides
the expected loss of range resolution caused by the longer pulse.
...
The sidelobes of the Barker 13 coded signal are not suited for weather radar applications. To try and mitigate those, amplitude
tapering is applied.
However, simulations show that it does not improve the signal. Due to the Barker codes being
perfect this is not surprising. Specifically, Figure \ref{fig:barker13-15us-opti} shows the optimization results of applying a 
Kaiser window with different values of $\beta$ to the Barker 13 signal. It shows that both ISL and PSL generally become worse with increased
tapering, while range resolution stays constant. This means that the mainlobe width is not affected at all by amplitude tapering.
As expected, the PPR shape is consistent with other results since it only depends on the Kaiser window.

The ISL does become slightly better with very light tapering, with the minimum ISL occuring with $\beta = 1.026$. The zero-Doppler cut of
the AF at this point is shown in Figure \ref{fig:barker13-15us-opti-slice3}. Unfortunately, sidelobes still occur at around \SI{-22}{\decibel},
so there is no improvement. These results show that a Barker coded signal is not very useful for weather radar applications, even with amplitude
tapering.
...
Figures \ref{fig:lfm-15us} and \ref{fig:lfm-200us} show the AFs of two LFM waveforms, with pulse lengths of \SI{15}{\micro\second} and
\SI{200}{\micro\second}, respectively.
...
As expected, the sidelobes are around \SI{-13}{\decibel} in both AFs, but also, the shapes are approximately the same
for each case, which means that the pulse length has little effect on the range resolution. Instead, range
resolution is determined by the sweep bandwidth, not the pulse length, as is expressed in Equation \eqref{eq:lfm-rangeres}.
...
As expected, the ISL dramatically decreases, as does the PSL. At the same time, the range resolution decreases from
an initial \SI{40}{\metre} to about \SI{200}{\metre}. The PPR may be the limiting metric here because the tapered signal quickly
...
PSL of \SI{-33}{\decibel}, and a range resolution of \SI{68}{\metre}.

The corresponding zero Doppler cut of the AF at this point is shown in figure \ref{fig:lfm-isl-15us-opti-slice13}. It 
clearly shows the sidelobes at about \SI{-70}{\decibel}, which means this waveform would be adequate for typical weather observations if the 
\SI{3}{\decibel} loss in power can be accepted.
...
Each function is applied over the pulse length $T$ of the signal.
...
in Section \ref{s:quantifications}. It is then determined which sweep function is best suited for weather radar applications, 
and further changes to the setup are made to combine it with amplitude tapering to try and achieve even better performance.


\subsection{Parameterized NLFM Results}
\label{s:nlfm-only-results}
Figure \ref{fig:nlfm-side-15us-opti} shows the results for varying NLFM waveforms in regards to ISL, PSL, and range resolution.
The closer the frequency modulation function approaches an S-curve, the better the ISL becomes, though it is never
fantastic. The maximum ISL is around \SI{-15}{\decibel}. The PSL decreases rapidly first to about 
\SI{-13}{\decibel} and then slowly decreases to about \SI{-20}{\decibel}, where it stays fairly constant. The range resolution
gets progressively worse, starting at about \SI{30}{\metre} and increasing to about \SI{220}{\metre}, meaning the mainlobe broadens.
This also explains the decreasing and eventually constant PSL, because the whole AF is one big mainlobe with only minimal sidelobes.
...
To get a better picture of the NLFM waveforms, consider Figure \ref{fig:nlfm-side-15us-0D-1}, showing the zero-Doppler slice of
...
The X and Y axes of the following figures
show the parameters of the Kaiser window, $\beta$, and NLFM sweep function, $a$, respectively. For each combination of $\beta$ and $a$,
a waveform and AF are generated and then evaluated.
...
tapered generally has better ISL performance. The same is true for the NLFM parameter $a$, i.e. for higher $a$, ISL is lower.
Next, the PSL shown in figure \ref{fig:nlfm-kaiser-15us-opti-psl} closely mirrors the shape of the ISL optimization,
...
impulse response of the amplifier. The effects of that on the waveform are presented here.
To provide a baseline, Figure \ref{fig:pulsewoir-15us} shows a rectangular pulse that is undistorted as a reference again.
