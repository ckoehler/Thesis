...
about \SI{-6}{\decibel}, which is significant but only occurs at high velocities and long pulses.
However, pulse lengths of \SI{200}{\micro\second} are not common. For comparison, the two pulse lengths used in the WSR-88D radar
...
\caption{ AF of a rectangular pulse of length 200 $\mu s$. The effect on Doppler
performance for typical velocities of weather phenomena is about \SI{6}{\decibel} loss, which is significant. }
...
\section{Barker Code}
In this section, a length 13 Barker code of different pulse lengths will be examined. Figures \ref{fig:barker13-15us}
and \ref{fig:barker13-15us-0D} show a \SI{15}{\micro\second} pulse that has been phase coded with a length 13 Barker
...
Comparing these images to Figures \ref{fig:barker13-200us} and \ref{fig:barker13-200us-0D}, there is again significant
difference in the Doppler dimension, so the pulse length does affect the shape of the AF for a Barker 13 code as well. The
distortion of the mainlobe is minimal and can be neglected, but there is some sidelobe distortion, elevating their level
to about \SI{-13}{\decibel}. The range delay is also larger due to the increased pulse length, as expected.
...
\caption{ AF of the Barker 13 code of length 200 $\mu s$. There is some sidelobe distortion for high Doppler velocities,
but the mainlobe level is not affected.}
...
\SI{15}{\micro\second} and \SI{200}{\micro\second}, respectively.  Comparing the two for differences shows that in the
...
pulse length has little effect on the range resolution, which is approximately \SI{30 }{\metre}. Instead, range
...
applied yet, so there is no loss of sensitivity. The general form of the function describing the frequency sweep is:
...
by the impulse response of the amplifier. The effects of that on the waveform are presented here. A couple of
assumptions have been made. First, since the exact impulse response is unknown, a Rayleigh-shaped approximation is used.
Second, it is assumed that the impulse response is as long as the pulse, in this case $\tau = \SI{15}{\micro\second}$,
which is likely not accurate. However, doing this will exaggerate the effect and make it more easily visible, which is
