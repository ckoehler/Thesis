...
the AF of a \SI{1}{\micro\second} pulse.  There is no significant change in the AF across the Doppler axis, so the different
...
but it is so minimal that it is minimal and the impact on weather radar applications is negligible.  Additionally,
...
\caption{ AF of a rectangular pulse of length 15 $\mu s$. There is no significant Doppler mismatch for typical velocities of
weather phenomena.}
\label{fig:simplepulse-15us}
\includegraphics[width=\imgsizes]{figures/pulse-200us.png}
\caption{ AF of a rectangular pulse of length 200 $\mu s$. There is still no significant effect on Doppler
performance for typical velocities of weather phenomena. }
...
\section{Barker Code}
In this section, a length 13 Barker code of different pulse lengths will be examined. Figures \ref{fig:barker13-15us} and
\ref{fig:barker13-15us-0D} show a \SI{15}{\micro\second} pulse that has been phase coded with a length 13 Barker code. As
for a rectangular pulse, there is no significant degradation of the signal with increasing Doppler shifts, so the
Doppler distortions can be neglected.  The zero-Doppler cut shows the typical shape of the Barker code and sidelobes of
about \SI{-22}{\decibel} \citep[p. 351]{Skolnik:2001}. The range resolution is approximately \SI{110 }{\metre} at the
\SI{3}{\decibel} point.

Comparing these images to Figures \ref{fig:barker13-200us} and \ref{fig:barker13-200us-0D} again shows no significant
...
\begin{figure}[H]
\includegraphics[width=\imgsizes]{figures/barker-15us.png}
\caption{ AF of the Barker 13 code of length 15 $\mu s$. The sidelobes are all at the same level, which is too high to
achieve sufficient sensitivity for typical weather applications. }
\label{fig:barker13-15us}
\includegraphics[width=\imgsizes]{figures/barker-15us-0D.png}
\caption{Zero Doppler cut of the AF of a 13 element Barker code. PSL is at about \SI{-22}{\decibel}, which is too high
for typical weather applications.}
\label{fig:barker13-15us-0D}
\end{figure}
...
The sidelobes of the Barker 13 coded signal are not suitable for typical weather radar applications. To try and mitigate those,
amplitude tapering is applied.  However, performance calculations show that it does not improve the signal. Due to the Barker codes
...
generally become worse with increased $\beta$, while range resolution stays constant. This means that the mainlobe
width is not affected at all by amplitude tapering. This is an interesting result, because a widening of the mainlobe is
usually the case, as explained in Section \ref{s:ampmod} and shown for other waveforms in this chapter.
As expected, the PPR shape is consistent with other results since it only depends on the Kaiser window.

The ISL does become slightly better with very light tapering, with the minimum ISL occurring with $\beta = 1.026$. The
...
The frequency of an LFM waveform is shown in Figure \ref{fig:lfm-fmf}. It is a linear sweep from \SI{-2.5}{\mega\hertz}
...
\caption{ AF of an LFM waveform of length 15 $\mu s$. Again, Doppler mismatch is negligible for typical weather-related
velocities.  }
\label{fig:lfm-15us}
\includegraphics[width=\imgsizes]{figures/lfm-200us.png}
\caption{ AF of an LFM waveform of length 200 $\mu s$. Increasing the pulse length has no effect on either Doppler
...
pulse length has little effect on the range resolution, which is approximately \SI{30 }{\metre}. Instead, range resolution is determined by the sweep bandwidth,
...
\caption{ Zero-Doppler cut of an LFM waveform AF of length \SI{15}{\micro\second}. Typical PSL is about \SI{-13}{\decibel}. }
\label{fig:lfm-15us-0d}
\includegraphics[width=\imgsizes]{figures/lfm-200us-0D.png}
\caption{ Zero-Doppler cut of an LFM waveform AF of length \SI{200}{\micro\second}. Range resolution is identical to the
...
tapering. In this work, a Kaiser window is applied to demonstrate the effects of amplitude tapering.  Based on Equation \eqref{eq:kaiser2}, the parameter $\beta$
is varied from 0 to 20 in approximately 0.2 increments, and the AF generated and evaluated for each by quantifying its performance according to the
...
quickly loses half of its energy with only minor amplitude tapering and eventually decreases to about \SI{-9}{\decibel}
of the original amplitude, which means the signal loses a significant amount of its energy. This, in turn, means reduced
sensitivity.
...
Clearly, a trade-off is made. If the half power point is selected, the waveform would have an ISL of about
...
observations for a \SI{3}{\decibel} loss in power. 
...
functions described by Equation \eqref{eq:nlfm-sweep} below. The PPR here is always 1 because no amplitude tapering is
applied yet, so there is no loss of sensitivity. The general form is:
...
to about \SI{220}{\metre}, meaning the mainlobe broadens.  This also explains the decreasing and eventually constant
PSL, because the whole AF is one big mainlobe with only minimal sidelobes.
...
with the sweep function with $a=6.46$.  Although the sidelobes are a little lower at \SI{-40}{\decibel} and the range
...
by the impulse response of the amplifier. The effects of that on the waveform are presented here.  
...
\caption{ An ideal AF for a rectangular pulse of length $\tau = \SI{15}{\micro\second}$. }
...
Figures \ref{fig:lfmwir-15us} and \ref{fig:lfmwir-15us} show the AFs of an LFM and NLFM waveform, respectively, both
...
Generally, then, a system impulse response has no significant effect on the Doppler distortion for weather radar
applications. However, it does affect the range delay by smoothing out the waveform similar to amplitude tapering,
lowering range resolution. The distortion is not symmetric because the system impulse response is not, either, which is
the most significant result presented here. Despite the benefits of smoothing and sidelobe reduction caused by the
impulse response, the asymmetry is highly undesirable and should be corrected.
