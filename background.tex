...
The spectrum of such a LFM waveform approaches the shape of a rectangle that is becomes better defined
...
If $u(t)$ has an added phase that changes quadratically, i.e. $u^{'}(t) = u(t) e^{j\pi kt^2}$, which 
...
each pulse is shifted in phase by $e^{jk \pi}$, where $k=(0, \frac{1}{2}, 1, \frac{3}{2})$. The performance of 
each code is measured by calculating the Integrated Sidelobe Level (ISL), which is defined as (\cite{Keeler:1999}):
