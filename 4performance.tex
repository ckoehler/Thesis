...
For amplitude weighting, another factor needs to be taken into consideration, i.e. Pulse Power Ratio. For a rectangular pulse, 100\% of the signal
power is transmitted. For a amplitude-tapered pulse, however, that is not the case. We define the Pulse Power Ratio as follows:
...
unambiguous velocity is \SI{80000}{\metre\per\second}.
At \SI{25}{\metre\per\second}, for example, a Doppler shift of $f_d = \frac{2v}{\lambda} = \frac{50}{0.1} = \SI{500}{Hz}$ is
observed.
...
moving at \SI{30}{\metre\per\second}, $0.001 \ll 1$ is true, so the Doppler effect can be ignored. We will see this confirmed later in the 
simulations.
...
which is usually hail. However, that means that weaker return signals, e.g. rain at 15 dBZ, only get about 20 \%
