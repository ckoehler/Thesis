...
For atmospheric weather radars, such as the WSR-88D with a wavelength of \SI{3000}{\mega\hertz}, and a target velocity of \SI{50}{\metre\per\second}, a Doppler frequency of $|f_\mathrm{d}| =
...
Next, the effects of the amplifier in the receive chain on the pulse will be examined. For simplification, the
components in the receive chain up to and excluding the matched filter are represented by some equivalent system with
impulse response $g(t)$, shown in Figure \ref{fig:distortblock}.  Starting with Equation \eqref{eq:AF} describing the
