...
metrics are used to quantify their performance. Together they can assist in creating and evaluating waveforms that
perform optimally for weather radars.

\subsection{Ambiguity Function}
\label{s:af}
The main metric to determine waveform performance is the Ambiguity Function (AF) \citep{Rihaczek:1968}. It is defined as:
...
where $u(t)$ is the complex envelope of the signal, $\tau$ is the delay due to propagation and $f_d$ is the Doppler frequency in the Doppler axis.
It can clearly be be be seen that, for $f_d = 0$, the AF reduces to the ACF, presented in Section \ref{s:2pc}.
...
Amplitude tapering or non-linear frequency modulation can also change the resolution.



\subsection{Integrated Sidelobe Level}
The performance of each code is measured by calculating the Integrated Sidelobe Level (ISL), which is defined as \citep{Keeler:1999}:
...
as the main focus is weather application. As such, the ISL gives the power ratio of sidelobes to mainlobe across the whole AF, with lower being
better.
...
that has overall low sidelobes except for one very dominant one may have a low ISL, but may not be performing well enough for actual use due
to the outlier sidelobe.
The Peak Sidelobe Level (PSL) is the ratio between the highest sidelobe and the mainlobe in the plane of the ambiguity function and so complements
the ISL. It is given by \citep{Keeler:1999}
...
For amplitude tapering, another factor needs to be taken into consideration, i.e. the Pulse Power Ratio (PPR). For a rectangular pulse, \SI{100}{\percent} of the signal
...
\begin{equation}
\label{PPR}
\text{PPR} = \frac{\int_{T} w(t)dt}{A_p T}, 
\end{equation}
where $w(t)$ is the tapering function applied to the pulse, integrated over the pulse length $T$, and $A_p$ is the original amplitude.
This metric represents the fraction of energy that is utilized when amplitude tapering is applied and can help quantify the loss in sensitivity
caused by tapering.
...
At that velocity, a Doppler frequency of $f_d = \frac{2v}{\lambda} = \frac{65}{0.1} = \SI{650}{Hz}$ is
observed.
...
moving at \SI{30}{\metre\per\second}, $0.001 \ll 1$ is true, so the Doppler effect can be ignored.
The higher the pulse compression ratio $BT$ is, the more mismatch will be caused by the Doppler shift.

All this said, the simulations in this work will show in Chapter \ref{chap:optimization} that for weather radar applications, the Doppler mismatch
can be safely ignored for the low velocities of most observed weather phenomena.
...
Starting with Equation \eqref{eq:AF}, convolve the return signal with $h(t)$, the impulse
...
It can be seen that applying the impulse response to the return signal is equivalent to applying it to the original pulse $u(t)$. 
This makes it easy in simulation to apply an impulse response to the calculations in order to evaluate the its effects on the AF.
The results of a Rayleigh-shaped impulse response on a rectangular pulse are presented in Section \ref{s:ireffects}.
...
amplitude of each sample is converted to one of a finite possible values for the amplitude \citep[p. 140]{Richards:2005}.
...
is \SI{93}{\decibel} \citep[p. 47]{Doviak:2006}. However, that means that weaker return signals must have a sufficient number of bits available to adequately
quantize any possible return signal.
For every additional bit, the dynamic range increases about \SI{6}{\decibel}, so 16 bits are needed to represent the whole
dynamic range \citep[p. 142]{Richards:2005}.
