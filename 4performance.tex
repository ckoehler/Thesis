...
Besides weather, other sources of distortions limit the performance of the waveform. In this section, another limitation
...
\begin{align*}
   \chi(\tau,f_\mathrm{d}) &= \left|\int_{-\infty}^\infty{s(t) \left[(s^*(t+\tau) e^{-j2\pi f_\mathrm{d} t})*h(t)\right]\ dt}\right| \\
   &= \left|\int_{-\infty}^\infty{s(t) \int_{-\infty}^{\infty}{s^*(t'+\tau) e^{-j2\pi f_\mathrm{d} t'}h^*(t'-t) dt'}\ dt}\right| \\
   &= \left|\int_{-\infty}^\infty{s^*(t'+\tau) e^{-j2\pi f_\mathrm{d} t'}\int_{-\infty}^{\infty}{s(t) h^*(t'-t) dt}\ dt'}\right| 
\end{align*}
