...
\label{s:af}
The main metric to determine waveform performance is the Ambiguity Function (AF) \citep{Rihaczek:1968}. It is defined
as:
...
pulse length used in other places, and $f_d$ is the Doppler frequency in the Doppler axis.  It can clearly be be be seen
that, for $f_d = 0$, the AF reduces to the ACF, presented in Section \ref{s:2pc}.
...
Another metric to evaluate the performance of a waveform is the 3dB range resolution in units of meters.  Lower value
means finer resolution of the waveform. The factors that influence range resolution are mainly pulse length and
amplitude tapering. For pulse length, the shorter the pulse, the better the resolution.  Amplitude tapering or
non-linear frequency modulation can also change the resolution.
...
performing well enough for actual use due to the outlier sidelobe.  The Peak Sidelobe Level (PSL) is the ratio between
the highest sidelobe and the mainlobe in the plane of the ambiguity function and so complements the ISL. It is given by
\citep{Keeler:1999}
...
\section{Limitations}
There are a some limitations that need to be taken into consideration, e.g. whether waveforms used in weather radar need
...
the maximum unambiguous velocity is \SI{32.5}{\metre\per\second}, based on $\lambda = \SI{0.1}{\metre}$ and a maximum
...
All this said, the simulations in this work will show in Chapter \ref{chap:optimization} that for weather radar
applications, the Doppler mismatch can be safely ignored for the low velocities of most observed weather phenomena.
...
amplitude of each sample is converted to one of a finite possible values for the amplitude \citep[p.
140]{Richards:2005}.

For $b$ available bits there are $2^b$ possible values to represent the amplitude of the signal. These $b$ bits need to
be able to contain the whole dynamic range of possible return signals. For the WSR-88D network radars, the dynamic range
at the receiver is \SI{93}{\decibel} \citep[p. 47]{Doviak:2006}. However, that means that weaker return signals must
have a sufficient number of bits available to adequately quantize any possible return signal.  For every additional bit,
the dynamic range increases about \SI{6}{\decibel}, so 16 bits are needed to represent the whole dynamic range \citep[p.
142]{Richards:2005}.
