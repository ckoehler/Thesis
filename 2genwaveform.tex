...
Figure \ref{fig:rangealiasing}, for example, if $r_a = \SI{100}{\kilo\metre}$, and there is a target at a distance of
\SI{150}{\kilo\metre} from the radar, it will appear to the radar to be only \SI{50}{\kilo\metre} away.  To increase
$r_a$, a longer PRT is needed.
...
velocity ``wrapping around'' and being detected wrongly. With $v_a= \SI{50}{\metre\per\second}$, a target moving at
...
TODO: describe dual prf, s-z coding. add subsection for these.


\section{Staggered PRT}
As mentioned earlier, the problem with a single PRT is the so-called Doppler dilemma. A short
PRT provides high aliasing velocity, while a long PRT gives us a long aliasing range. Neither gives us
both.  One way around this limitation is by using two different PRTs, a long and a short one, and combine measurements of both
to increase aliasing velocity.  Typically, the long PRT is chosen based on the desired aliasing range, while the
short PRT is chosen such that their combination maximizes the aliasing velocity. The aliasing velocity of two
...
TODO: is this dual prf or staggered prf?
TODO: add figure for Ts1 and Ts2
...
it with the complex conjugate of the coding sequence, $a^*_k$. For more details, refer to \cite{Sachidananda:1999}.

TODO: add more detail on s-z coding.
...
\end{equation}
The goal is to maximize $P_{av}$ while maintaining $T_s$ for a desired maximum range. 
...
amplifying it \citep[p. xxx]{Skolnik:2001}. The amplified signal is then delivered to the output cavity and further down the transmit chain of the
radar.
...
\begin{equation}
  \label{eq:mfsnr}
  \left(\frac{S}{N}\right)_{out} = \frac{|s_o(t_0)|^2}{n_o^2},
\end{equation}
where $s_o$ is the output signal of the filter at a certain delay $t_0$, and $n_o$ is the Gaussian output noise. The
variables influencing the output signal are the original signal $s(t)$ with delay $t_0$. The whole output
...
The total output noise power $n_o^2$ for white noise interference with spectral density $N_0/2$ is described by:

\begin{equation}
  \label{eq:mfnoise}
  n_o^2 = \frac{1}{2 \pi}\frac{N_0}{2} \int_{-\infty}^{\infty} |H(\omega)|^2 d\omega.
\end{equation}
...
For a simple rectangular pulse of the form $s(t)=\frac{1}{\sqrt{T}} \text{rect}(\frac{t}{T})$, the AF is \citep{Levanon:2004}:
...
\begin{align}
  |\chi(\tau,f_d)| &= 
  \begin{cases} 
  \left|\left(1-\frac{|\tau|}{T}\right) \frac{\sin[\pi T f_d(1- \frac{|\tau|}{T})]}{\pi T f_d(1-
  \frac{|\tau|}{T})}\right| & \text{for $|\tau| \leq T$}, \\
  0 & \text{otherwise}.
  \end{cases}
\end{align}
For $\tau=0$, the result is the $\mathrm{sinc}$ function, for $f_d=0$ it is simply the ACF of a pulse, which is
a triangle.
