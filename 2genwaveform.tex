...
$\mathrm{SNR}=\frac{2E}{N_\mathrm{0}}$.

With $h(t)$ known, the output of the matched filter is then 
\begin{equation}
  \label{eq:filterresponse}
  s_\mathrm{o}(t_\mathrm{0}) = s(t) \star h(t) = \int_{-\infty}^{\infty}{s(\tau) K s^*[t_\mathrm{0} - (t - \tau)] d\tau}.
\end{equation}
...
\begin{equation}
  \label{eq:acf}
  s_\mathrm{o}(0) = \mathrm{ACF}(s(t)) = \int_{-\infty}^{\infty}{s(\tau) s^*(\tau-t) d\tau}.
\end{equation}
...
\begin{equation}
\label{eq:AF}
  |\chi(\tau,f_\mathrm{d})| = \left| \int_{-\infty}^\infty{s(t) s^*(t+\tau) e^{-j2\pi f_\mathrm{d} t} dt}\right|,
\end{equation}
where $s(t)$ is the complex envelope of the signal and $\tau$ is the delay due to propagation, not to be confused with the
pulse length used in other places. It can clearly be seen that, for $f_\mathrm{d} = 0$, the
...
\begin{align*}
  ...
  &= 
  \begin{cases}
    \frac{1}{T} \int_{-\frac{T}{2} + \tau}^{\frac{T}{2}}{e^{j2\pi f_\mathrm{d} t}dt} & \text{for $0 \leq \tau \leq T$}, \\
    \frac{1}{T} \int_{-\frac{T}{2}}^{\frac{T}{2} + \tau}{e^{j2\pi f_\mathrm{d} t}dt} & \text{for $-T \leq \tau < 0$}, \\
    0 & \text{otherwise}. \\
  \end{cases}
\end{align*}
...
\begin{align}
  |\chi(\tau,f_\mathrm{d})| &= 
  \begin{cases} 
  \left|\left(1-\frac{|\tau|}{T}\right) \frac{\sin[\pi T f_\mathrm{d}(1- \frac{|\tau|}{T})]}{\pi T f_\mathrm{d}(1-
  \frac{|\tau|}{T})}\right| & \text{for $|\tau| \leq T$}, \\
  0 & \text{otherwise}.
  \end{cases}
\end{align}
For $\tau=0$, the result is the $\mathrm{sinc}$ function, for $f_\mathrm{d}=0$ it is simply the ACF of a pulse, which is
a triangle.
