...
component controls the performance of the waveform.}
...
velocity, while a long PRT gives us a large aliasing range. Neither gives us both.  One way around this limitation is by
...
TODO: describe ratio, cite Torres.
...
or even fourth trip echoes correctly. One such implementation is called SZ phase codes \cite{Sachidananda:1999}.  For this technique, every other
...
TODO: Rectangular Pulse only in the headline, put in dB.
\begin{figure}[h]
...
most of the energy in a narrow mainlobe with low sidelobes. 
...
TODO: simple system block diagram. SNR noise complex? What is it?
...
\end{equation}
where $S(\omega)$ is the Fourier transform of the input signal $s(t)$ and $H(\omega)$ the Fourier transform of $h(t)$, the impulse response
of the desired matched filter. The exponential describes the time delay $t_\mathrm{0}$ in the frequency domain.
...
\begin{equation}
  ...
  d\tau_\mathrm{d}}.
\end{equation}
TODO: compare to convolution. The concept of the ACF for a rectangular pulse is shown in Figure \ref{fig:acf}. The height of the triangle at any given
point is determined by the overlap of signal with the matched filter response, with the maximum occurring when both
exactly overlap.
...
\begin{align*}
  ...
  &= \left| \frac{1}{\tau j 2 \pi f_\mathrm{d}} e^{j 2 \pi f_\mathrm{d} t/2} \left\{ \exp\left[j 2 \pi f_\mathrm{d}
